...
def _get_named_provider(name: str):
    ...
    if provider is None:
        # Enforce the non-disableable free-only policy before constructing any
        # network client, so a paid endpoint can never be reached accidentally.
        enforce_free_provider(name, get_provider_config(name))
...
def _provider_candidates_for_fallback(primary_name: str) -> list[str]:
    """Return only providers permitted by the mandatory free-only policy."""
    try:
    ...
    for name in candidates:
        if not name or name == primary_name or name in seen:
            continue
        seen.add(name)
        try:
            enforce_free_provider(name, get_provider_config(name))
...
def chat(
    ...
    """Stream from a selected free-only provider with safe fallback."""
    ...
    if provider_name is not None:
        try:
            enforce_free_provider(provider_name, get_provider_config(provider_name))
        ...
        if not _provider_has_credentials(provider_name):
            ...
            if fallback is None:
                yield {
                    "type": "error",
                    "error": f"Provider '{provider_name}' is not configured for free use",
                    "content": f"Provider '{provider_name}' is not configured for free use",
                    "final": True,
                }
                return
            yield {
