...
def main():
    ...
    parser.add_argument("--ui", action="store_true", help="Launch the Friday desktop UI")
    ...
    print_colored(BANNER, "36")
    print_colored("─" * get_terminal_width(), "90")
    print_colored("Friday — AI Assistant", "33")
    print_colored("─" * get_terminal_width(), "90")
    print(f"Language: {LANG_LABELS.get(args.lang, args.lang)}")
    ...
    if args.voice:
        if not is_voice_available():
            print("Voice mode is unavailable on this system.")
            return
        print("Voice mode active. Say 'exit' to quit.")
        while True:
            text = listen(language=args.lang)
            if not text:
                continue
