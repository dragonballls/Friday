...
export const OrbCore = memo(function OrbCore({
  ...
  return (
    <div className="relative flex items-center justify-center w-full h-full min-w-0" style={{ zIndex: 1 }}>
      ...
      >
        <span
          className={`inline-block w-1.5 h-1.5 shrink-0 rounded-full ${isOnline ? 'animate-pulse-glow' : ''}`}
          style={{
          ...
          aria-hidden="true"
        />
        <span className="truncate text-[11px] tracking-[0.2em]" style={{ color: isOnline ? accent : C_TERTIARY }}>
          {isOnline ? (ORB_STATE_LABELS[orbState] ?? orbState.toUpperCase()) : 'OFFLINE'}
        </span>
        {location && <span className="truncate text-[10px] uppercase tracking-widest" style={{ color: C_TERTIARY }}>{location}</span>}
      </div>

      {chips.map(chip => (
        <div
          key={chip.id}
          className="pointer-events-none absolute flex max-w-[34%] min-w-0 items-center gap-1.5 px-2.5 py-1 rounded-md glass animate-fade-slide-up"
          style={{
        ...
        >
          <span className="shrink-0 text-[9px] tracking-[0.15em]" style={{ color: C_TERTIARY }}>{chip.label}</span>
          <span className="min-w-0 truncate text-[11px] font-mono" style={{ color: C_SECONDARY }}>{chip.value}</span>
        </div>
      ))}

      {latency > 0 && (
        <div
          className="pointer-events-none absolute bottom-[6%] left-1/2 -translate-x-1/2 px-2 py-0.5 rounded glass animate-fade-slide-up"
