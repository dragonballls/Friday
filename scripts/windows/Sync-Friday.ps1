...
try {
    if (-not $mutex.WaitOne(0)) { Write-Log 'another sync is already running; exiting'; exit 0 }
    ...
    if (-not $Force) {
        & git diff --quiet
        if ($LASTEXITCODE -ne 0) { Write-Log 'skipping: tracked working-tree changes detected'; exit 0 }
        & git diff --cached --quiet
        if ($LASTEXITCODE -ne 0) { Write-Log 'skipping: staged changes detected'; exit 0 }
    }
    ...
    if ($backendChanged) {
        ...
        if ($task) {
            Restart-ScheduledTask -TaskName 'Friday Backend'
        }
    }
